...
pub fn run() {
    ...
    for line in lines {
        ...
        for character in line.chars() {
            if character == '#' {
                energy_source.activate(Point::new(i, j, 0, 0));
            }
            j += 1;
        }
        i += 1;
...
impl<'a, 'b> Add<&'b Point> for &'a Point {
    ...
    fn add(self, right: &'b Point) -> Point {
        Point::new(self.x + right.x, self.y + right.y, self.z + right.z, self.w + right.w)
    }
}

impl Point {
    pub fn new(x: i32, y: i32, z: i32, w: i32) -> Point {
        Point { x, y, z, w }
    }

    fn neighbourghs() -> [Point; 80] {
        [
            Point::new(-1, -1, -1, -1),
            Point::new(-1, -1, -1, 0),
            Point::new(-1, -1, -1, 1),
