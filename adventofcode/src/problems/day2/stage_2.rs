...
fn is_valid(line: &PasswordLine) -> bool {
    let first = line.password.chars().nth(line.policy.from - 1).unwrap();
    let second = line.password.chars().nth(line.policy.to - 1).unwrap();
    return (first == line.policy.character) ^ (second == line.policy.character);
}
